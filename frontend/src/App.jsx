...
export default function App() {
  ...
  const [filename,      setFilename]    = useState("umowa-serwisowa (przykład)");
  ...
  const handleReanalyse = useCallback(() => {
    ...
    reanalyse((pages) => analyseDocument(pages, userPrompt))
      .catch((err) => setApiError(err.message ?? "Analiza nie powiodła się."));
  ...
  return (
    <div style={{ ...theme, background: "var(--bg)", height: "100vh", overflow: "hidden", display: "flex", flexDirection: "column", fontFamily: "'DM Sans', sans-serif" }}>
      <style>{`
      ...
      <div style={{ flex: 1, display: "flex", overflow: "hidden", minHeight: 0 }}>
        <div style={{ flex: 1, minHeight: 0, overflowY: "auto", background: isDark ? "#23272f" : "#e8e6e1", padding: "40px 32px" }}>
          <DocumentView
            pages={pages}
